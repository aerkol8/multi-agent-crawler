...
| Never crawl same page twice | URL normalization in src/webcrawler/utils.py + per-run discovery dedup in src/webcrawler/storage.py | python -m unittest discover -s tests -v (test_depth_limit_and_dedup) |
...
| search(query) returns public fields (word,url,origin,depth,freq,score) | src/webcrawler/models.py + src/webcrawler/storage.py + src/webcrawler/search.py | python main.py --db demo.db search "crawler" --json |
| Search while indexing is active | append-only filesystem logs + replay in src/webcrawler/storage.py | python -m unittest discover -s tests -v (test_search_returns_results_while_indexing_active) |
| Search contract semantics documented | readme.md + product_prd.md (`sortBy` + public score formula) | review docs and run python main.py --db demo.db search "crawler python" --json |
...
| Grader inspectability (`*.data`) | src/webcrawler/storage.py (bucketed `*.data` + mirrored `data/storage/*.data`) | python scripts/evaluate_submission.py --python <python-bin> (term data inspectability PASS) |
...
4. Confirm search output contains word/url/origin/depth/freq/score and score formula matches docs.
